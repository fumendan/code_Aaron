...
class Nic(BasicPlugin):

    def cmd_handler(self, debug):
        if debug:
            result = open('files/nic.out', 'r', encoding='utf-8').read()
            interfaces_info = self._interfaces_ip(result)
        else:
            interfaces_info = self.linux_interfaces(self.run_cmd)
